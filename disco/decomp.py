...
def get_min_root(tree, gene_to_species=lambda x: x, verbose=False):
    ...
    for node in tree.traverse_postorder():
        if node.is_leaf():
            ...
            node.d_score = (
                left.d_score + right.d_score + score(node.down, left.down, right.down)
            )
    ...
    min_score = (
        left.u_score
        + left.d_score
        + score(left.up.union(left.down), left.up, left.down)
    )
    ...
    for node in tree.traverse_preorder(leaves=False):
        if not node.skip:
            ...
            node.u_score = (
                parent.u_score + other.d_score + score(node.up, parent.up, other.down)
            )

            total_score = (
                node.u_score
                + node.d_score
                + score(node.up.union(node.down), node.up, node.down)
            )
        ...
        for i, line in enumerate(fi, 1):
            ...
            if args.verbose:
                ...
                if len(tree.root.s) < 2:
                    ...
                    outgroup = min(
                        (len(child.s), child.s) for child in tree.root.child_nodes()
                    )
                    print(
                        "Best root had score ",
                        score,
